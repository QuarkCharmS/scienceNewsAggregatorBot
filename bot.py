...
def run_anthro_digest(top_n: int | None = None) -> None:
    """Fetch, rank (top 3), and post the human anthropology digest."""
    channel = _require_env("TELEGRAM_CHANNEL_ID")
    logger.info("Starting anthropology digest run...")

    articles = filter_unseen(fetch_all_articles(feeds=ANTHROPOLOGY_FEEDS))
    if not articles:
        logger.warning("No new anthropology articles after deduplication — skipping.")
        return
    ...
    try:
        ...
        logger.error("Failed to post anthropology digest to Telegram: %s", exc)
        return

    mark_sent(top_articles)
    _set_last_run(LAST_RUN_ANTHRO_FILE)
    logger.info("Anthropology digest completed successfully.")
...
if __name__ == "__main__":
    ...
    elif "--anthro-check" in args:
        run_if_missed(lambda: run_anthro_digest(top_n=count), LAST_RUN_ANTHRO_FILE, "Anthropology digest")
